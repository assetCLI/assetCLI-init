...
describe("bonding-curve", async () => {
  ...
  it("SOme account", async () => {
    const [token0Mint, token1Mint] = getOrderedMints(
      new PublicKey("6rvU9CGQGB5t4K4V9PyfDhxJzgFLvs21xqL9KjoAekcp"),
      new PublicKey("3F9qUA6mhhoCgzrneeRLtu9BQcibTWkpUnzQXsGKcC17")
    );
    const newpoolState = anchor.web3.PublicKey.findProgramAddressSync(
      [
        Buffer.from("pool"),
        AMM_CONFIG_ID.toBuffer(),
        token0Mint.toBuffer(),
    ...
    const new_lp_mint = PublicKey.findProgramAddressSync(
      [Buffer.from("pool_lp_mint"), newpoolState.toBuffer()],
      program.programId
    );
    console.log("new_lp_mint", new_lp_mint[0].toBase58());
    const acc = getAssociatedTokenAddressSync(
      new_lp_mint[0],
      new PublicKey("7a2ZbSypL8t9kR8ARMQRMXbPDQT91mVVqDsGFpU7gv28"),
      true
    );
    console.log("acc", acc.toBase58());
  });
});
